...
class Common
{
    ...
    public static function updateUserAmount($userId, $warehouseId)
    {
        if ($userId) {
            ...
            // Batch all order sums and counts into a single grouped query (was 8 separate queries)
            $orderStats = Order::where('user_id', $user->id)
                ->where('warehouse_id', $warehouseId)
                ->whereIn('order_type', ['purchases', 'purchase-returns', 'sales', 'sales-returns'])
                ->selectRaw('order_type, SUM(total) as total_sum, COUNT(*) as order_count')
                ->groupBy('order_type')
                ->get()
                ->keyBy('order_type');

            $totalPurchaseAmount      = (float) ($orderStats->get('purchases')?->total_sum ?? 0);
            $totalPurchaseReturnAmount = (float) ($orderStats->get('purchase-returns')?->total_sum ?? 0);
            $totalSalesAmount         = (float) ($orderStats->get('sales')?->total_sum ?? 0);
            $totalSalesReturnAmount   = (float) ($orderStats->get('sales-returns')?->total_sum ?? 0);

            $purchaseOrderCount      = (int) ($orderStats->get('purchases')?->order_count ?? 0);
            $purchaseReturnOrderCount = (int) ($orderStats->get('purchase-returns')?->order_count ?? 0);
            $salesOrderCount         = (int) ($orderStats->get('sales')?->order_count ?? 0);
            $salesReturnOrderCount   = (int) ($orderStats->get('sales-returns')?->order_count ?? 0);

            // Batch payment sums into a single grouped query (was 2 separate queries)
            $paymentStats = Payment::where('user_id', $user->id)
                ->where('warehouse_id', $warehouseId)
                ->whereIn('payment_type', ['in', 'out'])
                ->selectRaw('payment_type, SUM(amount) as amount_sum')
                ->groupBy('payment_type')
                ->get()
                ->keyBy('payment_type');

            $totalPaidAmountByUser = (float) ($paymentStats->get('in')?->amount_sum ?? 0);
            $totalPaidAmountToUser = (float) ($paymentStats->get('out')?->amount_sum ?? 0);
    ...
    public static function updateWarehouseHistory($type, $typeObject, $action = "delete", $skipAmountUpdate = false)
    {
        if ($type == 'order') {
    ...
    public static function storeAndUpdateOrder($order, $oldOrderId, $deferAmountUpdate = false)
    {
        $request = request();
        ...
        return self::storeAndUpdateOrderItem($order, $productItems, $oldOrderId, $deferAmountUpdate);
    }

    public static function storeAndUpdateOrderItem($order, $productItems, $oldOrderId = "", $deferAmountUpdate = false)
    {
        $tsi = microtime(true);
        ...
        if (count($productItems) > 0) {
            ...
            // Pre-load ProductDetails for all new items grouped by warehouse — avoids N individual queries
            $preloadByWarehouse = [];
            foreach ($productItems as $pi) {
                $pi = (object) $pi;
                if ($pi->item_id == '' || $pi->item_id == null) {
                    $wh = (isset($pi->warehouse_xid) && $pi->warehouse_xid)
                        ? self::getIdFromHash($pi->warehouse_xid)
                        : ($order->order_type == 'stock-transfers' ? $order->from_warehouse_id : $order->warehouse_id);
                    $pid = self::getIdFromHash($pi->xid);
                    $preloadByWarehouse[$wh][] = $pid;
                }
            }
            $productDetailsCache = [];
            foreach ($preloadByWarehouse as $wh => $pids) {
                $rows = ProductDetails::withoutGlobalScope('current_warehouse')
                    ->where('warehouse_id', $wh)
                    ->whereIn('product_id', array_unique($pids))
                    ->get();
                foreach ($rows as $row) {
                    $productDetailsCache[$wh . '_' . $row->product_id] = $row;
                }
            }
            ...
            foreach ($productItems as $productItem) {
                ...
                if ($productItem->item_id == '' || $productItem->item_id == null) {
                    ...
                    $productDetails = $productDetailsCache[$warehouseId . '_' . $productId] ?? null;
        ...
        // Update Customer or Supplier total amount, due amount, paid amount
        // Skip if caller will handle it after payments are saved (deferred for POS)
        if (!$deferAmountUpdate) {
